...
Right-click acts on what is under the pointer or selected: **Sao chép** (Copy, with a selection) · **Dán** (Paste) · **Chọn tất cả** (Select all) · on a URL **Mở liên kết** / on a file **Mở tệp**, **Mở bằng app mặc định**, **Chèn @đường-dẫn vào CLI** / on a folder **Mở thư mục** · **Sao chép liên kết / đường dẫn** · **Tìm vùng đã bôi** · **Tìm trong terminal**. Tab-level actions sit in the editor title bar (top-right of the tab): icons for **Phiên mới** (New session), **Khởi động lại phiên** (Restart) and **Tìm** (Find), and under its **…** menu: **Đổi tên tab** (Rename, `F2`), **Sao chép ngữ cảnh** (Copy context), **Mở lại phiên cũ** (Resume past session), **Lệnh nhanh** (Quick command). Hovering a link shows what `Cmd/Ctrl + click` will open and the resolved path.
